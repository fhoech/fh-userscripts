...
window.resizeableTF =
{
	...
	pageload: function (e) {
		...
		var TF = []
		for (var selector in resizeableTF.TF_tags) {
			var parts = selector.split('.'),
				tagName = parts[0],
				className = parts[1];
			var elems = resizeableTF.doc.getElementsByTagName(tagName);
			if (className) {
				var origElems = elems;
				elems = []
				for (var i = 0; i < origElems.length; i++) {
					if (origElems[i].className.split(/\s+/).indexOf(className) > -1) {
						elems.push(origElems[i]);
					}
				}
			}
			TF = TF.concat(Array.prototype.slice.call(elems));
		}
		if (TF.length) {
	...
	getlineheight: function (elem) {
		var style = document.defaultView.getComputedStyle(elem, null);
		return style ? parseFloat(style.lineHeight) : 0;
	},
	...
	resizeta_v: function (e) {
		var curPos = resizeableTF.getposition(resizeableTF.curTF_Nr);
		if (!resizeableTF.TF_lineheights[resizeableTF.curTF_Nr]) return;
